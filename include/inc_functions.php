...
function add_column_if_not_exist($table, $column, $column_attr = null ){
  global $db;
    ...
    $exists = false;
    $columns = mysqli_query($db,"show columns from $table");
    ...
    if(!$exists){
        mysqli_query($db,"ALTER TABLE ".$table." ADD ".$column." ".$column_attr);
    }
}
/*
...
function table_exists($table){
    ...
    while($c = mysqli_fetch_array($tables)){
        if($c[0] == $table){
            $exists = true;
            break;
        }
...
function ordered_list($array,$menu_parent = 0)
{
  $temp_array = array();
  foreach($array as $element)
  {
    ...
    {
      $element['subs'] = $ordered_list($array, $element['menu_id']);
      $temp_array[] = $element;
    }
  }
  return $temp_array;
